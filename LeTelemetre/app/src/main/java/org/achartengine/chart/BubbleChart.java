...
public class BubbleChart extends XYChart {
    ...
    protected ClickableArea[] clickableAreasForPoints(List<Float> points, List<Double> values, float yAxisValue, int seriesIndex, int startIndex) {
        ...
        for (int i = 0; i < length; i += MIN_BUBBLE_SIZE) {
            ...
            float f = (float) size;
            ret[i / MIN_BUBBLE_SIZE] = new ClickableArea(new RectF(((Float) points.get(i)).floatValue() - ((float) size), ((Float) points.get(i + 1)).floatValue() - ((float) size), ((float) size) + ((Float) points.get(i)).floatValue(), ((Float) points.get(i + 1)).floatValue() ), ((Double) values.get(i)).doubleValue(), ((Double) values.get(i + 1)).doubleValue());
        }
        return ret;
    }
