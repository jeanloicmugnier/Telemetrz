...
public class BarChart extends XYChart {
    ...
    protected ClickableArea[] clickableAreasForPoints(List<Float> points, List<Double> values, float yAxisValue, int seriesIndex, int startIndex) {
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            if (this.mType == Type.STACKED) {
                ...
                ret[i / 2] = new ClickableArea(new RectF(startX, Math.min(y, yAxisValue), (2.0f * halfDiffX) + startX, Math.max(y, yAxisValue)), ((Double) values.get(i)).doubleValue(), ((Double) values.get(i + 1)).doubleValue());
            }
        }
        return ret;
    }
    ...
    public void drawSeries(Canvas canvas, Paint paint, List<Float> points, SimpleSeriesRenderer seriesRenderer, float yAxisValue, int seriesIndex, int startIndex) {
        ...
        for (int i = 0; i < length; i += 2) {
            float x = ((Float) points.get(i)).floatValue();
            drawBar(canvas, x, yAxisValue, x, ((Float) points.get(i + 1)).floatValue(), halfDiffX, seriesNr, seriesIndex, paint);
        }
        paint.setColor(seriesRenderer.getColor());
    }
    ...
    protected void drawChartValuesText(Canvas canvas, XYSeries series, SimpleSeriesRenderer renderer, Paint paint, List<Float> points, int seriesIndex, int startIndex) {
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            if (!isNullValue(value)) {
                ...
                if (value >= 0.0d) {
                    drawText(canvas, getLabel(renderer.getChartValuesFormat(), value), x, ((Float) points.get(i + 1)).floatValue() - renderer.getChartValuesSpacing(), paint, 0.0f);
                } else {
                    drawText(canvas, getLabel(renderer.getChartValuesFormat(), value), x, ((((Float) points.get(i + 1)).floatValue() + renderer.getChartValuesTextSize()) + renderer.getChartValuesSpacing()) - 3.0f, paint, 0.0f);
                }
            }
        }
    }
